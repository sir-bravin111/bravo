            ...
            Qr_Code_By_Ibrahim_Adams.ev.on("connection.update", async (s) => {
                ...
                if (qr && !responseSent) {
                    ...
                    <head>
                        ...
                        <title>WHATSAPP BOT - Advanced QR Scanner</title>
                ...
                if (connection === "open") {
                    ...
                    try {
                        await Qr_Code_By_Ibrahim_Adams.sendMessage(Qr_Code_By_Ibrahim_Adams.user.id, {
                            text: `✅ *QR Connection Established!*\n\nPreparing your long-term session...\n\nPlease wait while we optimize your connection for extended use.`
                        });
                        console.log(`✅ QR connection message sent`);
                    } catch (sendError) {
                        console.log(`⚠️ Failed to send QR connection message:`, sendError.message);
                    }
                    ...
                    // EXTENDED SESSION GENERATION - 70 SECONDS FOR LONG QR SESSION
                    console.log(`⏳ Generating long-term QR session (70 seconds)...`);
                    
                    // Phase 1: Initial stabilization (15 seconds)
                    await delay(15000);
                    ...
                    // Phase 2: Data collection (35 seconds)
                    await delay(35000);
                    ...
                    // Phase 3: Final optimization (20 seconds)
                    await delay(20000);
                    ...
                    try {
                        ...
                        if (fs.existsSync(sessionPath)) {
                            ...
                            console.log(`✅ Long-term QR session enhanced: ${JSON.stringify(sessionData).length} bytes`);
                        }
                    } catch (readError) {
                    ...
                    if (!sessionData) {
                        ...
                        try {
                            await Qr_Code_By_Ibrahim_Adams.sendMessage(Qr_Code_By_Ibrahim_Adams.user.id, {
                                text: `❌ Session creation failed. Please try scanning QR again.`
                            });
                        } catch (e) {}
                        return;
                    }
                    ...
                    console.log(`🗜️ Compressing long-term QR session data...`);
                    let compressedData = zlib.gzipSync(JSON.stringify(sessionData), { level: 9 });
                    let b64data = compressedData.toString('base64');
                    
                    // Create enhanced session string
                    let fullSessionData = `BWM-XMD-LONG-TERM-QR-SESSION-v2::${Date.now()}::${b64data}::END`;
                    console.log(`✅ Long QR session compressed: ${fullSessionData.length} characters`);

                    // SEND COMPLETE SESSION DIRECTLY VIA WHATSAPP (NO GITHUB)
                    try {
                        // Send completion message first
                        await Qr_Code_By_Ibrahim_Adams.sendMessage(Qr_Code_By_Ibrahim_Adams.user.id, {
                            text: `🎉 *LONG-TERM SESSION READY!*\n\nYour optimized QR session has been prepared for extended use.\n\n⚡ *Features:*\n• High-performance connection\n• Extended duration\n• Enhanced stability\n• Auto-recovery\n• QR-based authentication`
                        });
                        ...
                        try {
                            ...
                            await Qr_Code_By_Ibrahim_Adams.sendMessage(Qr_Code_By_Ibrahim_Adams.user.id, {
                                text: `📦 *Session Parts ${chunks.length}*`
                            });
                            ...
                            await Qr_Code_By_Ibrahim_Adams.sendMessage(Qr_Code_By_Ibrahim_Adams.user.id, {
                                text: `✅ *All session chunks sent!*\n\nCombine them in order (1 to ${chunks.length}) to get your complete session.`
                            });
                    ...
                    try {
                        ...
                        console.log(`🎉 LONG-TERM QR SESSION CREATED AND SENT`);
                    } catch (cleanupError) {
